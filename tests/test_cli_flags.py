...
def test_generate_cwl_workflow() -> None:
    ...
    yaml_path = str(pathlib.Path(__file__).parent.resolve() / "data/wic/bbbc.wic")
    ...
    with open("autogenerated/bbbc.cwl", "r") as cwl_file:
        result_dict = yaml.safe_load(cwl_file)

    with open(
        str(pathlib.Path(__file__).parent.resolve() / "data/cwl/bbbc.cwl"), "r"
    ) as cwl_file:
