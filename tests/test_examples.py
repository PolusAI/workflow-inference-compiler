...
def test_examples() -> None:
    ...
    for yml_path_str in yml_paths:
        ...
        yaml_tree_raw = wic.ast.read_AST_from_disk(YamlTree(yml_path_str, root_yaml_tree), yml_paths, tools_cwl, validator)
...
def test_cwl_embedding_independence() -> None:
    ...
    for yml_path_str in yml_paths:
        ...
        Path('autogenerated/').mkdir(parents=True, exist_ok=True)
        yaml_tree_raw = wic.ast.read_AST_from_disk(YamlTree(yml_path_str + '.yml', root_yaml_tree), yml_paths, tools_cwl, validator)
        ...
        for sub_node_data, sub_yaml_forest in zip(node_data_lst[1:], yaml_forest_lst):
            ...
            if not sub_cwl_embedded == sub_cwl_fakeroot:
                ...
                with open(f'{sub_name}_forest_embedded.yml', 'w') as w:
                    w.write(yaml.dump(yaml_forest))
                with open(f'{sub_name}_forest_fakeroot.yml', 'w') as w:
