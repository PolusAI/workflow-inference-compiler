...
import wic.cli
import wic.compiler
import wic.inlineing
import wic.input_output
...
def run_workflows(yml_path_str: str, yml_path: Path, cwl_runner: str, args: argparse.Namespace, docker_pull_only: bool = False) -> None:
    ...
    with open(f'autogenerated/{Path(yml_path).stem}_tree_python_script.yml', mode='w', encoding='utf-8') as f:
        f.write(yaml.dump(yaml_tree.yml))
    ...
    if args.cwl_inline_subworkflows:
        while True:
            ...
            namespaces_list = wic.inlineing.get_inlineable_subworkflows(yaml_tree, tools_cwl, False, [])
            if namespaces_list == []:
                break

            yaml_tree, _len_substeps = wic.inlineing.inline_subworkflow(yaml_tree, namespaces_list[0])
    ...
    rose_tree = wic.plugins.cwl_prepend_dockerFile_include_path_rosetree(rose_tree)
    wic.input_output.write_to_disk(rose_tree, Path('autogenerated/'), relative_run_path=True)
    ...
    if not args.no_docker_remove_entrypoints:
        ...
        # tools_cwl_noent = plugins.dockerPull_append_noentrypoint_tools(tools_cwl)
        # # Now compile again with tools_cwl_noent
        # compiler_info = compiler.compile_workflow(yaml_tree, args, [], [subgraph], {}, {}, {}, {},
        #                                           tools_cwl_noent, True, relative_run_path=True, testing=False)
        # # So we can get an updated rose_tree
        # rose_tree = compiler_info.rose
        # (Alternatively, just update rose_tree directly)
        rose_tree = wic.plugins.dockerPull_append_noentrypoint_rosetree(rose_tree)
        wic.input_output.write_to_disk(rose_tree, Path('autogenerated/'), relative_run_path=True)

    if args.allow_partial_failures:
        rose_tree = wic.plugins.cwl_update_outputs_optional_rosetree(rose_tree)
        wic.input_output.write_to_disk(rose_tree, Path('autogenerated/'), relative_run_path=True)
...
def test_cwl_embedding_independence(yml_path_str: str, yml_path: Path) -> None:
    ...
    yaml_forest_lst = [yf for yf in yaml_forest_lst if ".yml" not in yf.yaml_tree.step_id.stem]
    ...
    node_data_lst = [node for node in node_data_lst if ".yml" in node.name[0]]

    # This test doesn't necessarily need to write to disk, but useful for debugging.
    wic.input_output.write_to_disk(rose_tree, Path('autogenerated/'), relative_run_path=False)
...
def test_inline_subworkflows(yml_path_str: str, yml_path: Path) -> None:
    ...
    namespaces_list = wic.inlineing.get_inlineable_subworkflows(yaml_tree, tools_cwl, 'backend' in wic_tag, [])
    if namespaces_list == []:
    ...
    for namespaces in namespaces_list:
        inline_yaml_tree, _len_substeps = wic.inlineing.inline_subworkflow(yaml_tree, namespaces)
