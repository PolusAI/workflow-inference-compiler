...
def main() -> None:
    ...
    yaml_forest = ast.read_yml_forest((yaml_path, root_yaml_tree), {}, yml_paths, tools_cwl)
    # Write the combined workflow (with all subworkflows as children) to disk.
    with open(f'autogenerated/{Path(yaml_path).stem}_tree.yml', 'w') as f:
    ...
    with rootgraph.subgraph(name=f'cluster_{yaml_path}') as subgraph:
        ...
        workflow_data = compiler.compile_workflow(yaml_forest[0], args, [], [subgraph], {}, {}, tools_cwl, True, relative_run_path=True)
        recursive_data = workflow_data[0]

    utils.write_to_disk(recursive_data, Path('autogenerated/'), relative_run_path=True)
    ...
    if args.cwl_run_local:
        ...
        cmd = ['cwltool', '--cachedir', 'cachedir','--outdir', 'outdir', f'autogenerated/{yaml_stem}.cwl', f'autogenerated/{yaml_stem}_inputs.yml']
