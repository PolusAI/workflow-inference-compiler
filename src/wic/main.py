...
class NoResolvedFilter(logging.Filter):
    def filter(self, record: logging.LogRecord) -> bool:
        ...
        return not (True if m else False)
...
def validate_cwl(cwl_path_str: str, skip_schemas: bool) -> None:
    ...
    loadingContext, workflowobj, uri = cwltool.load_tool.fetch_document(cwl_path_str)
    loadingContext, uri = cwltool.load_tool.resolve_and_validate_document(
        loadingContext, workflowobj, uri, preprocess_only=False, skip_schemas=skip_schemas
    )
    ...
    process_ = cwltool.load_tool.make_tool(uri, loadingContext)
    return None # ignore process_ for now
...
def main() -> None:
    ...
    tools_cwl = get_tools_cwl(args.cwl_dirs_file, args.validate_plugins,
    ...
    with rootgraph.subgraph(name=f'cluster_{yaml_path}') as subgraph_gv:
        ...
        try:
            ...
            print(f'See error_{yaml_path.stem}.txt for detailed technical information.')
            # Do not display a nasty stack trace to the user; hide it in a file.
            with open(f'error_{yaml_path.stem}.txt', mode='w', encoding='utf-8') as f:
    ...
    if args.run_local:
        ...
        if (not (proc.returncode == 0 and hello_output in output)) and (not args.ignore_docker):
