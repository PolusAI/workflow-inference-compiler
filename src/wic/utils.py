...
def reindex_wic_steps(wic_steps: Yaml, index: int, count: int = 1) -> Yaml:
    ...
    Args:
        ...
        count (int): The integer to add to the step numbers.
    ...
    for keystr, val in wic_steps.items():
        ...
        newstr = f'({i+count}, {s})' if i >= index else keystr
