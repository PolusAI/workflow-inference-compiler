...
class Workflow:
    ...
    def compile(
        ...
        if out_dir:
            ...
            shutil.copy(
                autogenerated_path.with_name(self.name + "_inputs.yml"),
                output.with_name(self.name + "_inputs.yml"),
            )
            print(f"Successfully compiled to {str(output)}")


# s1 = Step(Path("/Users/camilovelezr/axle/workflow/viz_share/assembler.cwl"))
# s1.stitchPath = Path("/Users/camilovelezr/axle/workflow")
# s1.imgPath = Path("/Users/camilovelezr/axle/workflow")
# s1.timesliceNaming = False
