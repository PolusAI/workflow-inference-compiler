...
class Workflow:
    ...
    def compile(self) -> None:
        ...
        print(proc.stdout)
        print(proc.stderr)
        print(f"Successfully compiled to {str(self.path)}")
    ...
    def run(self) -> None:
        ...
        os.system(f"cwltool {workflow_path} {inputs_path}")

    # def run(self) -> None:
    #     """Run compiled workflow."""
    #     print(f"Running {self.name}")
    #     workflow_path = self.path.joinpath("autogenerated", f"{self.name}.cwl")
    #     inputs_path = self.path.joinpath("autogenerated", f"{self.name}_inputs.yml")
    #     try:
    #         proc = subprocess.run(
    #             args=["cwltool", "--relax-path-checks", workflow_path, inputs_path],
    #             capture_output=True,
    #             cwd=self.path,
    #             check=True,
    #             text=True,
